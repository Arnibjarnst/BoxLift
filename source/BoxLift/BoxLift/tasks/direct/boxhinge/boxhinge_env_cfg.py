...
class BoxhingeEnvCfg(DirectRLEnvCfg):
    ...
    # Seconds to hold the final trajectory pose after phase reaches end, for endpoint stability.
    post_traj_hold_s: float = 2.0

    # Probability of forcing phase=0 on reset; compensates for RSI undersampling trajectory start.
    ...
    obs_obj_pos_noise = 0.001            # m, per-step Gaussian noise on box position
    obs_obj_ori_noise = 0.001            # rad, per-step noise on box orientation

    # per-episode bias (held constant per episode; models camera-to-robot calibration error)
    obs_obj_pos_bias_std = 0.005          # m
    obs_obj_ori_bias_std = 0.005          # rad
    ...
    # "sum": Gaussian kernels summed; "product": exp(-β_pos·d_pos)·exp(-β_rot·d_rot)
    ...
    # DexMachina-style VOC: PD wrench drives the box along the reference while gain decays
    # as the policy meets reward thresholds.
    voc_enabled: bool = True
    voc_kp_pos: float = 1000.0    # N/m
    voc_kp_rot: float = 100.0     # Nm/rad
    voc_kp_min: float = 10        # below this, kp/kv set to zero
